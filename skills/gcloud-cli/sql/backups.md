...
| `--description` | DESCRIPTION |  | A friendly description of the backup. |
| `--location` | LOCATION |  | Choose where to store your backup. Backups are stored in the closest multi-region location to you by default. Only customize if needed. |


[Official reference](https://cloud.google.com/sdk/gcloud/reference/sql/backups/create)
...
[Official reference](https://cloud.google.com/sdk/gcloud/reference/sql/backups/delete)
...
**Synopsis:**
```
gcloud sql backups describe ID [--instance=INSTANCE, -i INSTANCE]
    [GCLOUD_WIDE_FLAG ...]
```
...
[Official reference](https://cloud.google.com/sdk/gcloud/reference/sql/backups/describe)
...
| Flag | Value | Default | Description |
|------|-------|---------|-------------|
| `--instance` | INSTANCE, -i INSTANCE |  | Cloud SQL instance ID or "-" for all instances. |


[Official reference](https://cloud.google.com/sdk/gcloud/reference/sql/backups/list)
...
| Flag | Value | Default | Description |
|------|-------|---------|-------------|
| `--backup-description` | BACKUP_DESCRIPTION |  | _[At least one of these must be specified:]_ Provides description for the backup going to be taken. |


[Official reference](https://cloud.google.com/sdk/gcloud/reference/sql/backups/patch)
...
| Flag | Value | Default | Description |
|------|-------|---------|-------------|
| `--restore-instance` | RESTORE_INSTANCE |  | The ID of the target Cloud SQL instance that the backup is restored to. |
...
| `--backup-project` | BACKUP_PROJECT |  | The project of the instance to which the backup belongs. If it isn't specified, backup and restore instances are in the same project. This flag is not supported when restore happens from backup name, only supported when restore happens from backup ID in timestamp format. |
...
| `--clear-disk-encryption` | CLEAR_DISK_ENCRYPTION |  | Disables CMEK in the restored instance. |
| `--clear-network` |  |  | Clears the network setting. This is useful to restore a backup to a different project or region where the original network configuration isn't available. |
...
| `--insights-config-query-insights-enabled` |  |  | Enable query insights feature to provide query and query plan analytics. |
...
| `--insights-config-record-application-tags` |  |  | Allow application tags to be recorded by the query insights feature. |
...
[Official reference](https://cloud.google.com/sdk/gcloud/reference/sql/backups/restore)
