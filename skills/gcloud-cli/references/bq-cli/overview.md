...
> **Do not confuse with `gcloud bq`.** The GA `gcloud bq` command group covers *only* BigQuery Migration Service workflows (`gcloud bq migration-workflows` — 4 commands). Everything else BigQuery on the CLI is this standalone `bq` tool. See [`../bq/overview.md`](../bq/overview.md) for the `gcloud bq` surface.
...
- **`gcloud bq` (Migration Service only):** [`../bq/overview.md`](../bq/overview.md) — the separate `gcloud bq migration-workflows` group.
