...
The standalone BigQuery CLI (invoked as plain `bq`, not `gcloud bq`). Format: `bq [--global_flags] <command> [--command_flags] [args]`. Flags below are command-specific; global flags (`--project_id`, `--location`, `--format`, `--quiet`, `--synchronous_mode`, ...) are covered in [`SKILL.md`](SKILL.md). Boolean flags accept `--flag`/`--flag=true`/`--flag=false`/`--noflag`. Flags marked *(Experimental)*, *(Preview)*, or *(Deprecated)* in the tool's own help are labeled as such — prefer the unlabeled GA surface.
...
Interactive bootstrap for credentials and the config file. The official reference recommends **not** using `bq init` to create `.bigqueryrc` — write the file with an editor instead (see [`SKILL.md`](SKILL.md)). Not listed on the web reference command list; documented via `bq help`.
